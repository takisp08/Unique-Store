...
const jointocreate1map = new Map();
      ...
      resolve(2);
        try{
          ...
          for (let i = 0; i < channels.length; i++) {
            ...
            if (jointocreate1map.get(key)) {
              var vc = guild.channels.cache.get(jointocreate1map.get(key));
              if (vc.members.size < 1) {
                jointocreate1map.delete(key);
                return vc.delete();
              } else {}
            }
          }
...
//voice state update event to check joining/leaving channels
  ...
  if (!oldState.channelID && newState.channelID) {
    if(newState.channelID !== config.JOINTOCREATECHANNEL1) return;  //if its not the jointocreate1channel skip
    jointocreate1channel(newState);   //load the function
  }
  // LEFT V12
  if (oldState.channelID && !newState.channelID) {
            //get the jointocreate1channel id from the map
          if (jointocreate1map.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
            //fetch it from the guild
            var vc = oldState.guild.channels.cache.get(jointocreate1map.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`));
            //if the channel size is below one
            if (vc.members.size < 1) { 
              //delete it from the map
              jointocreate1map.delete(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`); 
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    async function jointocreate1channel(user) {
      //log it 
      ...
      await user.guild.channels.create(`💸Donate`, {
        type: 'voice',
        parent: "821450541740720128", //or set it as a category id
      }).then(async vc => {
        //move user to the new channel
        user.setChannel(vc);
        //set the new channel to the map
        jointocreate1map.set(`tempvoicechannel_${vc.guild.id}_${vc.id}`, vc.id);
